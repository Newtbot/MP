$(document).ready(async function () {
	...
	//monthly chart
	const monthlyCtx = document.getElementById("MonthlyDataChart").getContext("2d");
	...
	function updateWeeklyChart(metric) {
		...
		app.api.get(queryParams, function (error, data) {
			...
			for (let weekNumber in weeklyData) {
				const averageValue = weeklyData[weekNumber].reduce((acc, val) => acc + val, 0) / weeklyData[weekNumber].length;
	...
	function updateMonthlyChart(metric) {
		...
		app.api.get(queryParams, function (error, data) {
			...
			for (let monthNumber in monthlyData) {
				const averageValue = monthlyData[monthNumber].reduce((acc, val) => acc + val, 0) / monthlyData[monthNumber].length;
	...
	//week buttons
	document.getElementById("weeklypsiButton").addEventListener("click", function () {
		updateWeeklyChart("psi")
	});

	document.getElementById("weeklytempButton").addEventListener("click", function () {
		updateWeeklyChart("temperature");
	});

	document.getElementById("weeklyhumButton").addEventListener("click", function () {
		updateWeeklyChart("humidity");
	});

	document.getElementById("weeklyo3Button").addEventListener("click", function () {
		updateWeeklyChart("o3");
	});

	document.getElementById("weeklyno2Button").addEventListener("click", function () {
		updateWeeklyChart("no2");
	});

	document.getElementById("weeklyso2Button").addEventListener("click", function () {
		updateWeeklyChart("so2");
	});

	document.getElementById("weeklycoButton").addEventListener("click", function () {
		updateWeeklyChart("co");
	});

	//month buttons
	document.getElementById("monthlypsiButton").addEventListener("click", function () {
		updateMonthlyChart("psi");
	});

	document.getElementById("monthlytempButton").addEventListener("click", function () {
		updateMonthlyChart("temperature");
	});

	document.getElementById("monthlyhumButton").addEventListener("click", function () {
		updateMonthlyChart("humidity");
	});

	document.getElementById("monthlyo3Button").addEventListener("click", function () {
		updateMonthlyChart("o3");
	});

	document.getElementById("monthlyno2Button").addEventListener("click", function () {
		updateMonthlyChart("no2");
	});

	document.getElementById("monthlyso2Button").addEventListener("click", function () {
		updateMonthlyChart("so2");
	});

	document.getElementById("monthlycoButton").addEventListener("click", function () {
